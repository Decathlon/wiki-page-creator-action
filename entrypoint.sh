...
if [ -z "$WIKI_REPO_URL" ]; then
  echo "WIKI_REPO_URL ENV is missing. Cannot proceed"
  exit 1
fi
...
git pull https://${GH_PAT}@github.com/$WIKI_REPO_URL
cd ..
...
git push --set-upstream https://${GH_PAT}@github.com/$WIKI_REPO_URL master
